...
module.exports = {
  async averageVehicles(user, personas) {
    ...
    let some = parseFloat(
      await getPriceVehilhe(vehicle_user[0].CHASSI, vehicle_user[0].PLACA),
    )
    let index = 1
    ...
    for (persona in personas) {
      ...
      const vehicle_persona =
        vehicles_persona.struct_RespostaRst.Resposta.VEICULOS.ITEM

      some += parseFloat(
        await getPriceVehilhe(
          vehicle_persona[0].CHASSI,
          vehicle_persona[0].PLACA,
        ),
      )
      index++
    }
